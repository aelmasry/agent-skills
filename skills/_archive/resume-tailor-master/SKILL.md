...
## Local routing (Ali / Cursor)

Load **`ali-career-profile`** before inventing any content.
Prefer this skill when Ali needs DOCX/PDF from a maintained master profile under `docs/resume/master-profile.md` (create on first run from Ali's CV).
For fast match + cover letter + 7-day plan without DOCX, use **`cv-jd-matcher`**.
